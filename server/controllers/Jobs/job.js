                            ...
                            .then(data => {
                                return audio_transcribe.transcriber(data.files.slice(1, data.files.length), transcriptions, this.language)
                                    .then(transcriptions => {
                                        ...
                                        var durations_sliced = data.durations.slice(1, data.durations.length)
                                        console.log(durations_sliced[1].start - data.durations[0].end, durations_sliced[1].end - data.durations[0].end)
                                        return transcriptions.map((transcript, index) => {
                                            return {
                                                transcription: transcript,
                                                start: durations_sliced[index].start - data.durations[0].end,
                                                
                                                end: durations_sliced[index].end - data.durations[0].end
